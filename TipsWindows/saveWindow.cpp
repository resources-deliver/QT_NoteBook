...
// 窗口函数: 保存文件对话框窗口(单个文件)
QString TipsWindows::saveWindow(QString title, QString choicetipstext, QString choicetipsbutton){
    ...
    setwindow.setFileMode(QFileDialog::AnyFile);  // 设置文件对话窗口为允许保存单个任何文件
    setwindow.setAcceptMode(QFileDialog::AcceptSave);  // 设置文件对话窗口为接受保存模式
    setwindow.setNameFilter(tr("Text Files (*.txt);;All Files (*)"));  // 设置文件对话窗口的可保存文件类型
    QString choiceopenpath = "D:/Code/Qt(QtCreator)/NoteBookProject/TxtPackage/";  // 定义文件对话窗口的默认保存路径
    setwindow.setDirectory(choiceopenpath);  // 设置文件对话窗口的默认保存路径
